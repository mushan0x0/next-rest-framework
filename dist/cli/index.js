...
var writeOpenApiSpec = async ({
  ...
  try {
    if (!(0, import_fs2.existsSync)((0, import_path2.join)(process.cwd(), "public"))) {
      console.info(
    ...
    (0, import_fs2.writeFileSync)(path, jsonSpec, null);
    console.info(import_chalk4.default.green("OpenAPI spec generated successfully!"));
  } catch (e) {
    console.error(import_chalk4.default.red(`Error while generating the API spec: ${e}`));
  }
};
var generate = async ({ configPath }) => {
  ...
  try {
    ...
    if (!(JSON.stringify(openApiSpec) === JSON.stringify(spec))) {
      ...
      console.info(
        import_chalk4.default.green("OpenAPI spec up to date, skipping generation.")
      );
    }
  } catch {
    console.info(
      import_chalk4.default.yellowBright("No OpenAPI spec found, generating `openapi.json`...")
    );
    await writeOpenApiSpec({ path, spec });
  }
};
...
var validate = async ({ configPath }) => {
  ...
  const spec = await generateOpenApiSpec({ config });
  const path = (0, import_path3.join)(process.cwd(), "public", config.openApiJsonPath);
  try {
    const data = (0, import_fs3.readFileSync)(path);
    const openApiSpec = JSON.parse(data.toString());
    if (!(JSON.stringify(openApiSpec) === JSON.stringify(spec))) {
      console.error(
      ...
      console.info(import_chalk5.default.green("OpenAPI spec up to date!"));
      return true;
    }
  } catch {
    console.error(
...
program.command("generate").option(
  ...
  try {
    console.info(import_chalk6.default.yellowBright("Generating OpenAPI spec..."));
    await generate({
      configPath
    });
  } catch (e) {
...
program.command("validate").option(
  ...
  const configPath = options.configPath ?? "";
  try {
    console.info(import_chalk6.default.yellowBright("Validating OpenAPI spec..."));
